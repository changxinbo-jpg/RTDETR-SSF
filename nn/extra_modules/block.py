...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C3_SFConv', 'C2f_SFConv', 'C3_SPAB', 'C2f_SPAB', 'BasicBlock_SFConv', 'BottleNeck_SFConv',
           'C3_UniRepLKNetBlock', 'C2f_UniRepLKNetBlock', 'C3_DRB', 'C2f_DRB', 'BasicBlock_DRB', 'BottleNeck_DRB',
           'DWRC3_DRB', 'C3_DWR_DRB', 'C2f_DWR_DRB', 'SFConvC3', 'DRBC3', 'DBBC3', 'C2f_DBB', 'C3_DBB', 'BasicBlock_DBB', 'BottleNeck_DBB', 'CSP_EDLAN',
           ...
           'RepNCSPELAN4', 'DBBNCSPELAN4', 'OREPANCSPELAN4', 'DRBNCSPELAN4', 'SFConvNCSPELAN4', 'ADown', 'Fusion',
...
class SPAB(nn.Module):
    def __init__(self,
        ...
        self.in_channels = in_channels
        self.c1_r = SFConv(in_channels, in_channels, gain=2, s=1)
        self.c2_r = SFConv(in_channels, in_channels, gain=2, s=1)
        self.c3_r = SFConv(in_channels, out_channels, gain=2, s=1)
...
class Bottleneck_SFConv(Bottleneck):
    """Standard bottleneck with SFConv."""

    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        if k[0] == 3:
            self.cv1 = SFConv(c1, c_)
        self.cv2 = SFConv(c_, c2)

class C3_SFConv(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_SFConv(c_, c_, shortcut, g, k=(1, 3), e=1.0) for _ in range(n)))

class C2f_SFConv(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_SFConv(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class BasicBlock_SFConv(BasicBlock):
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        super().__init__(ch_in, ch_out, stride, shortcut, act, variant)
        
        self.branch2a = SFConv(ch_in, ch_out, s=stride)
        self.branch2b = SFConv(ch_out, ch_out)

class BottleNeck_SFConv(BottleNeck):
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        super().__init__(ch_in, ch_out, stride, shortcut, act, variant)
        
        self.branch2b = SFConv(ch_out, ch_out, s=stride)

class SFConvC3(RepC3):
    def __init__(self, c1, c2, n=3, e=1):
        super().__init__(c1, c2, n, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*[SFConv(c_, c_) for _ in range(n)])
...
class SFConvNBottleneck(RepNBottleneck):
    def __init__(self, c1, c2, kernel_size, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = SFConv(c1, c_)
...
class SFConvNCSP(RepNCSP):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(SFConvNBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class SFConvNCSPELAN4(RepNCSPELAN4):
    def __init__(self, c1, c2, c3, c4, c5=1):
        super().__init__(c1, c2, c3, c4, c5)
        self.cv2 = nn.Sequential(SFConvNCSP(c3//2, c4, c5), Conv(c4, c4, 3, 1))
        self.cv3 = nn.Sequential(SFConvCNCSP(c4, c4, c5), Conv(c4, c4, 3, 1))
